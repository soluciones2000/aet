...
async function cardnew(numero,nombre,monto) {
  ...
  msj1.innerHTML = 'Generando tarjeta, por favor espere...'; //Generating card, please wait ...
  ...
  .then(() => {
    ...
    // msj1.style.display = 'block';
    msj1.innerHTML = 'Transacción no confirmada, tarjeta no generada.'; //Transaction not confirmed, card not generated.
  ...
  if (validamonto(montotrx, montotrx.value)) {
    ...
    if (card1.saldo-trx>=0) {
      ...
      await contractCall('consumo', [card1.idcard, trx], 0).then(() => {
        ...
        // Transaction not confirmed, consumption was not recorded.
      });
    } else {
      msj2.innerHTML = 'El monto del consumo supera el saldo disponible';
      // The amount of consumption exceeds the available balance
      montotrx.value = '';
      montotrx.focus();
    }
